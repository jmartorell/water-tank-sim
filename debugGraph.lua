...
function debugGraph:new(type, x, y, width, height, delay, label, font)
  ...
  -- Build base data
  for _ = 0, math.floor(instance.width / 2) do
    ...
    if dt > self.delay or lastTime > self._time then
      ...
      if val == nil then
        if self._type == 'fps' then
          ...
          self.label = string.format("FPS: %3.1f", val)
        elseif self._type == 'mem' then
          ...
          self.label = string.format("Memory (KB): %3.0f", val)
        else
      ...
      self._max = max
      --]]
      self._max = math.max(unpack(self.data))
    end
  end
